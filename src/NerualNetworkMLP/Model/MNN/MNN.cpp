...
MatrixNerualNetwork::MatrixNerualNetwork(unsigned int numHiddenLayers)
    : _func(typeFunction::SIGMOIND),
      _neuronWeightMat(0),
      _biosWeightMat(0),
      _valueNeruals(numHiddenLayers + 2),
      _valueErrors(numHiddenLayers + 1) {
...
void MatrixNerualNetwork::setWeightMatrix() {
  Matrix inputLayer(static_cast<int>(TypeLayer::HIDDEN),
                    static_cast<int>(TypeLayer::INPUT));
  inputLayer.setRandom();
  _neuronWeightMat.push_back(inputLayer);
  for (unsigned int i = 0; i < _numOfHiddenLayers - 1; i++) {
    Matrix hiddenLayer(static_cast<int>(TypeLayer::HIDDEN),
                       static_cast<int>(TypeLayer::HIDDEN));
    hiddenLayer.setRandom();
    _neuronWeightMat.push_back(hiddenLayer);
  }
  Matrix hiddenLayer(static_cast<int>(TypeLayer::OUTPUT),
                     static_cast<int>(TypeLayer::HIDDEN));
  hiddenLayer.setRandom();
  _neuronWeightMat.push_back(hiddenLayer);
}
...
void MatrixNerualNetwork::forwardPropagation(const Image& image) {
  ...
  for (unsigned int i = 1; i <= _numOfHiddenLayers + 1; ++i) {
    _valueNeruals[i] =
        _neuronWeightMat[i - 1] * _valueNeruals[i - 1] + _biosWeightMat[i - 1];
  ...
  for (unsigned int i = 0; i < _numOfHiddenLayers + 1; ++i) {
    for (int j = 0; j < _neuronWeightMat[i].getRows(); ++j) {
      for (int k = 0; k < _neuronWeightMat[i].getCols(); ++k) {
        _neuronWeightMat[i].setValue(
            j, k,
            _neuronWeightMat[i](j, k) -
                lr / numOfEpoch * _valueNeruals[i](k, 0) *
                    _valueErrors[i](j, 0) *
                    _func.useDerivative(_valueNeruals[i + 1](j, 0)));
      }
      _biosWeightMat[i].setValue(
          j, 0,
          _biosWeightMat[i](j, 0) -
              lr / numOfEpoch * _valueErrors[i](j, 0) *
                  _func.useDerivative(_valueNeruals[i + 1](j, 0)));
    }
  }
}

void MatrixNerualNetwork::train(const Dataset& data, const Dataset& dataTest,
                                double percentTestData, int numOfEpoch) {
...
double MatrixNerualNetwork::test(const Dataset& data, double percentTestData) {
  _metrics.reset();
