#include "Neuron.h"
namespace s21 {
Neuron::Neuron(TypeLayer numOfWeights, typeFunction type)
    : _value(0), _weights(static_cast<int>(numOfWeights)), _func(type) {
...
void Neuron::backPropagation(int answer) { _error = (_value - answer); }

int Neuron::getNumOfWeights() { return _weights.size(); }

double Neuron::value() const { return _value; }

void Neuron::setValue(double value) { _value = value; }

double Neuron::sum() const { return _sum; }

double Neuron::error() const { return _error; }

void Neuron::setError(double sumError) { _error = sumError; }

double Neuron::bios() { return _bias; }
void Neuron::calcToEditBias(double lr) { _bias -= _error * _func.useDerivative(_value) * lr; }

double Neuron::weight(int i) const { return _weights[i]; }
