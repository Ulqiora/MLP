...
enum ExportPen {
    ...
    ,
    epAllowCosmetic  ///< Cosmetic pens are exported normally (e.g. in PDF exports, cosmetic pens always
                     ///< appear as 1 pixel on screen, independent of viewer zoom level)
};
...
enum AntialiasedElement {
    ...
    ,
    aeScatters = 0x0080  ///< <tt>0x0080</tt> Scatter symbols of plottables (excluding scatter symbols of
                         ///< type ssPixmap)
    ,
...
enum PlottingHint {
    ...
    ,
    phFastPolylines = 0x001  ///< <tt>0x001</tt> Graph/Curve lines are drawn with a faster method. This
                             ///< reduces the quality especially of the line segment
                             ///<                joins, thus is most effective for pen sizes larger than 1.
                             ///<                It is only used for solid line pens.
    ,
    phImmediateRefresh =
        0x002  ///< <tt>0x002</tt> causes an immediate repaint() instead of a soft update() when
    ...
    ,
    phCacheLabels = 0x004  ///< <tt>0x004</tt> axis (tick) labels will be cached as pixmaps, increasing
                           ///< replot performance.
};
...
enum Interaction {
    ...
    ,
    iRangeDrag = 0x001  ///< <tt>0x001</tt> Axis ranges are draggable (see \ref QCPAxisRect::setRangeDrag,
                        ///< \ref QCPAxisRect::setRangeDragAxes)
    ,
    ...
    ,
    iMultiSelect = 0x004  ///< <tt>0x004</tt> The user can select multiple objects by holding the modifier
                          ///< set by \ref QCustomPlot::setMultiSelectModifier while clicking
    ,
    iSelectPlottables = 0x008  ///< <tt>0x008</tt> Plottables are selectable (e.g. graphs, curves, bars,...
                               ///< see QCPAbstractPlottable)
    ,
    iSelectAxes =
    ...
    ,
    iSelectItems = 0x040  ///< <tt>0x040</tt> Items are selectable (Rectangles, Arrows, Textitems, etc. see
                          ///< \ref QCPAbstractItem)
    ,
    ...
    iSelectPlottablesBeyondAxisRect =
        0x100  ///< <tt>0x100</tt> When performing plottable selection/hit tests, this flag extends the
               ///< sensitive area beyond the axis rect
};
...
enum SelectionRectMode {
    srmNone  ///< The selection rect is disabled, and all mouse events are forwarded to the underlying
             ///< objects, e.g. for axis range dragging
    ,
    srmZoom  ///< When dragging the mouse, a selection rect becomes active. Upon releasing, the axes that are
             ///< currently set as range zoom axes (\ref QCPAxisRect::setRangeZoomAxes) will have their
             ///< ranges zoomed accordingly.
    ,
    srmSelect  ///< When dragging the mouse, a selection rect becomes active. Upon releasing, plottable data
               ///< points that were within the selection rect are selected, if the plottable's selectability
               ///< setting permits. (See  \ref dataselection "data selection mechanism" for details.)
    ,
    srmCustom  ///< When dragging the mouse, a selection rect becomes active. It is the programmer's
               ///< responsibility to connect according slots to the selection rect's signals (e.g. \ref
 ...
 public:
    ...
    */
    enum PainterMode {
        ...
        ,
        pmVectorized = 0x01  ///< <tt>0x01</tt> Mode for vectorized painting (e.g. PDF export). For example,
                             ///< this prevents some antialiasing fixes.
        ,
        ...
        ,
        pmNonCosmetic =
            0x04  ///< <tt>0x04</tt> Turns pen widths 0 to 1, i.e. disables cosmetic pens. (A cosmetic pen is
                  ///< always drawn with width 1 pixel in the vector image/pdf viewer, independent of zoom.)
    };
    Q_ENUMS(PainterMode)
...
class QCP_LIB_DECL QCPLayer : public QObject {
 ...
 public:
    ...
    */
    enum LayerMode {
        lmLogical  ///< Layer is used only for rendering order, and shares paint buffer with all other
                   ///< adjacent logical layers.
        ,
...
class QCP_LIB_DECL QCPLayoutElement : public QCPLayerable {
 ...
 public:
    ...
    */
    enum UpdatePhase {
        upPreparation  ///< Phase used for any type of preparation that needs to be done before margin
    ...
    */
    enum SizeConstraintRect {
        ...
        ,
        scrOuterRect  ///< Minimum/Maximum size constraints apply to outer rect, thus include layout element
                      ///< margins
    };
...
class QCP_LIB_DECL QCPLayoutGrid : public QCPLayout {
 ...
 public:
    ...
    */
    enum FillOrder {
        foRowsFirst  ///< Rows are filled first, and a new element is wrapped to the next column if the row
                     ///< count would exceed \ref setWrap.
        ,
        foColumnsFirst  ///< Columns are filled first, and a new element is wrapped to the next row if the
                        ///< column count would exceed \ref setWrap.
    };
    Q_ENUMS(FillOrder)
...
class QCP_LIB_DECL QCPLineEnding {
 ...
 public:
    /*!
      ...
      and \ref setLength. Some decorations like \ref esDisc, \ref esSquare, \ref esDiamond and \ref esBar
      only support a width, the length property is ignored.
    ...
    */
    enum EndingStyle {
        ...
        ,
        esHalfBar  ///< A bar perpendicular to the line, pointing out to only one side (to which side can be
                   ///< changed with \ref setInverted)
        ,
...
class QCPLabelPainterPrivate {
 ...
 protected:
    ...
    struct LabelData {
        ...
        QTransform transform;  // the transform about the label anchor which is at (0, 0). Does not contain
                               // final absolute x/y positioning on the plot/axis
    ...
    QColor mColor;
    int mPadding;
    double mRotation;  // this is the rotation applied uniformly to all labels, not the heterogeneous
                       // rotation in amCircularRotated mode
    bool mSubstituteExponent;
    QChar mMultiplicationSymbol;
    bool mAbbreviateDecimalPowers;
    // non-property members:
    QCustomPlot *mParentPlot;
    QByteArray mLabelParameterHash;  // to determine whether mLabelCache needs to be cleared due to changed
                                     // parameters
    ...
    virtual QByteArray generateLabelParameterHash()
        const;  // TODO: get rid of this in favor of invalidation flag upon setters?
 ...
 public:
    ...
    */
    enum TickStepStrategy {
        tssReadability  ///< A nicely readable tick step is prioritized over matching the requested number of
                        ///< ticks (see \ref setTickCount)
        ,
        tssMeetTickCount  ///< Less readable tick steps are allowed which in turn facilitates getting closer
                          ///< to the requested tick count
    };
...
class QCP_LIB_DECL QCPAxisTickerFixed : public QCPAxisTicker {
 ...
 public:
    ...
    */
    enum ScaleStrategy {
        ssNone  ///< Modifications are not allowed, the specified tick step is absolutely fixed. This might
                ///< cause a high tick density and overlapping labels if the axis range is zoomed out.
        ,
        ssMultiples  ///< An integer multiple of the specified tick step is allowed. The used factor follows
                     ///< the base class properties of \ref setTickStepStrategy and \ref setTickCount.
        ,
...
class QCP_LIB_DECL QCPAxisTickerPi : public QCPAxisTicker {
 ...
 public:
    ...
    */
    enum FractionStyle {
        fsFloatingPoint  ///< Fractions are displayed as regular decimal floating point numbers, e.g. "0.25"
                         ///< or "0.125".
        ,
        fsAsciiFractions  ///< Fractions are written as rationals using ASCII characters only, e.g. "1/4" or
                          ///< "1/8"
        ,
        fsUnicodeFractions  ///< Fractions are written using sub- and superscript UTF-8 digits and the
                            ///< fraction symbol.
    };
...
class QCP_LIB_DECL QCPAxis : public QCPLayerable {
    ...
    Q_PROPERTY(
        SelectableParts selectedParts READ selectedParts WRITE setSelectedParts NOTIFY selectionChanged)
 ...
 public:
    ...
    */
    enum ScaleType {
        ...
        ,
        stLogarithmic  ///< Logarithmic scaling with correspondingly transformed axis coordinates (possibly
                       ///< also \ref setTicker to a \ref QCPAxisTickerLog instance).
    };
    Q_ENUMS(ScaleType)
    /*!
 ...
 public:
    ...
    */
    enum ScatterShape {
        ...
        ,
        ssDot  ///< \enumimage{ssDot.png} a single pixel (use \ref ssDisc or \ref ssCircle if you want a
               ///< round shape with a certain radius)
        ,
        ...
        ,
        ssDisc  ///< \enumimage{ssDisc.png} a circle which is filled with the pen's color (not the brush as
                ///< with ssCircle)
        ,
        ...
        ,
        ssTriangleInverted  ///< \enumimage{ssTriangleInverted.png} an equilateral triangle, standing on
                            ///< corner
        ,
...
void QCPDataContainer<DataType>::add(const QCPDataContainer<DataType> &data) {
    ...
    } else  // don't need to prepend, so append and merge if necessary
    {
        ...
        if (oldSize > 0 &&
            !qcpLessThanSortKey<DataType>(
                *(constEnd() - n - 1), *(constEnd() - n)))  // if appended range keys aren't all greater than
...
void QCPDataContainer<DataType>::removeBefore(double sortKey) {
    ...
    mPreallocSize += int(itEnd - it);  // don't actually delete, just add it to the preallocated block (if it
...
void QCPDataContainer<DataType>::remove(double sortKey) {
    ...
    if (it != end() && it->sortKey() == sortKey) {
        if (it == begin())
            ++mPreallocSize;  // don't actually delete, just add it to the preallocated block (if it gets too
...
*/
template <class DataType>
typename QCPDataContainer<DataType>::const_iterator QCPDataContainer<DataType>::findBegin(
    ...
    if (expandedRange && it != constBegin())  // also covers it == constEnd case, and we know --constEnd is
                                              // valid because mData isn't empty
        --it;
    return it;
}
...
*/
template <class DataType>
typename QCPDataContainer<DataType>::const_iterator QCPDataContainer<DataType>::findEnd(
    double sortKey, bool expandedRange) const {
    if (isEmpty()) return constEnd();
...
QCPRange QCPDataContainer<DataType>::keyRange(bool &foundRange, QCP::SignDomain signDomain) {
    ...
    {
        if (DataType::sortKeyIsMainKey())  // if DataType is sorted by main key (e.g. QCPGraph, but not
                                           // QCPCurve), use faster algorithm by finding just first and last
                                           // key with non-NaN value
        {
        ...
        } else  // DataType is not sorted by main key, go through all data points and accordingly expand
                // range
        {
            while (it != itEnd) {
                if (!qIsNaN(it->mainValue())) {
...
void QCPDataContainer<DataType>::performAutoSqueeze() {
    ...
    } else if (totalAlloc > 1000)  // below 10 MiB raw data be generous with preallocated memory, below 1k
                                   // points don't even bother
    {
    ...
    if (shrinkPreAllocation || shrinkPostAllocation) squeeze(shrinkPreAllocation, shrinkPostAllocation);
}
...
class QCP_LIB_DECL QCPAbstractPlottable : public QCPLayerable {
 ...
 public:
    ...
    // introduced virtual methods:
    virtual double selectTest(const QPointF &pos, bool onlySelectable, QVariant *details = nullptr) const
        Q_DECL_OVERRIDE = 0;  // actually introduced in QCPLayerable as non-pure, but we want to force
    ...
    */
    enum PositionType {
        ptAbsolute  ///< Static positioning in pixels, starting from the top left corner of the
                    ///< viewport/widget.
        ,
        ptViewportRatio  ///< Static positioning given by a fraction of the viewport size. For example, if
                         ///< you call setCoords(0, 0), the position will be at the top left corner of the
                         ///< viewport/widget. setCoords(1, 1) will be at the bottom right corner,
                         ///< setCoords(0.5, 0) will be horizontally centered and vertically at the top of
                         ///< the viewport/widget, etc.
        ,
        ptAxisRectRatio  ///< Static positioning given by a fraction of the axis rect size (see \ref
                         ///< setAxisRect). For example, if you call setCoords(0, 0), the position will be at
                         ///< the top left corner of the axis rect. setCoords(1, 1) will be at the bottom
                         ///< right corner, setCoords(0.5, 0) will be horizontally centered and vertically at
                         ///< the top of the axis rect, etc. You can also go beyond the axis rect by
...
class QCP_LIB_DECL QCustomPlot : public QWidget {
    ...
    Q_PROPERTY(
        Qt::KeyboardModifier multiSelectModifier READ multiSelectModifier WRITE setMultiSelectModifier)
    ...
    */
    enum RefreshPriority {
        ...
        rpQueuedRefresh  ///< Replots immediately, but queues the widget repaint, by calling
                         ///< QWidget::update() after the replot. This way multiple redundant widget repaints
                         ///< can be avoided.
        ,
        rpRefreshHint  ///< Whether to use immediate or queued refresh depends on whether the plotting hint
                       ///< \ref QCP::phImmediateRefresh is set, see \ref setPlottingHints.
        ,
        rpQueuedReplot  ///< Queues the entire replot for the next event loop iteration. This way multiple
                        ///< redundant replots can be avoided. The actual replot is then done with \ref
...
PlottableType *QCustomPlot::plottableAt(const QPointF &pos, bool onlySelectable, int *dataIndex) const {
    ...
    double resultDistance =
        mSelectionTolerance;  // only regard clicks with distances smaller than mSelectionTolerance as
                              // selections, so initialize with that value

    foreach (QCPAbstractPlottable *plottable, mPlottables) {
        PlottableType *currentPlottable = qobject_cast<PlottableType *>(plottable);
        if (!currentPlottable ||
            (onlySelectable &&
             !currentPlottable->selectable()))  // we could have also passed onlySelectable to the selectTest
                                                // function, but checking here is faster, because we have
...
ItemType *QCustomPlot::itemAt(const QPointF &pos, bool onlySelectable) const {
    ...
    foreach (QCPAbstractItem *item, mItems) {
        ...
        if (!currentItem ||
            (onlySelectable &&
             !currentItem->selectable()))  // we could have also passed onlySelectable to the selectTest
                                           // function, but checking here is faster, because we have access
                                           // to QCPAbstractItem::selectable
            continue;
        if (!currentItem->clipToAxisRect() ||
            currentItem->clipRect().contains(pos.toPoint()))  // only consider clicks inside axis cliprect of
...
class QCPAbstractPlottable1D : public QCPAbstractPlottable,
                               public QCPPlottableInterface1D  // no QCP_LIB_DECL, template class ends up in
                                                               // header (cpp included below)
{
...
QCPDataSelection QCPAbstractPlottable1D<DataType>::selectTestRect(const QRectF &rect,
    ...
    if ((onlySelectable && mSelectable == QCP::stNone) || mDataContainer->isEmpty()) return result;
    ...
    QCPRange keyRange(
        key1, key2);  // QCPRange normalizes internally so we don't have to care about whether key1 < key2
    ...
    if (DataType::sortKeyIsMainKey())  // we can assume that data is sorted by main key, so can reduce the
                                       // searched key interval:
    {
    ...
    for (typename QCPDataContainer<DataType>::const_iterator it = begin; it != end; ++it) {
        ...
        {
            result.addDataRange(QCPDataRange(currentSegmentBegin, int(it - mDataContainer->constBegin())),
                                false);
    ...
    if (currentSegmentBegin != -1)
        result.addDataRange(QCPDataRange(currentSegmentBegin, int(end - mDataContainer->constBegin())),
                            false);
...
double QCPAbstractPlottable1D<DataType>::selectTest(const QPointF &pos, bool onlySelectable,
    ...
    if ((onlySelectable && mSelectable == QCP::stNone) || mDataContainer->isEmpty()) return -1;
    ...
    for (typename QCPDataContainer<DataType>::const_iterator it = begin; it != end; ++it) {
        ...
        if (keyRange.contains(mainKey) &&
            valueRange.contains(
                mainValue))  // make sure data point is inside visible range, for speedup in cases where sort
                             // key isn't main key and we iterate over all points
        {
...
void QCPAbstractPlottable1D<DataType>::getDataSegments(QList<QCPDataRange> &selectedSegments,
    ...
    if (mSelectable == QCP::stWhole)  // stWhole selection type draws the entire plottable with selected
                                      // style if mSelection isn't empty
    {
        if (selected())
...
void QCPAbstractPlottable1D<DataType>::drawPolyline(QCPPainter *painter,
    ...
    if (mParentPlot->plottingHints().testFlag(QCP::phFastPolylines) &&
        painter->pen().style() == Qt::SolidLine && !painter->modes().testFlag(QCPPainter::pmVectorized) &&
        ...
        while (i < lineDataSize) {
            if (qIsNaN(lineData.at(i).y()) || qIsNaN(lineData.at(i).x()) ||
                qIsInf(lineData.at(i).y()))  // NaNs create a gap in the line. Also filter Infs which make
                                             // drawPolyline block
            {
...
class QCP_LIB_DECL QCPColorGradient {
 ...
 public:
    ...
    */
    enum NanHandling {
        nhNone  ///< NaN data points are not explicitly handled and shouldn't occur in the data (this gives
    ...
    */
    enum GradientPreset {
        ...
        ,
        gpIon  ///< Half hue spectrum from black over purple to blue and finally green (creates banding
               ///< illusion but allows more precise magnitude estimates)
        ,
        gpThermal  ///< Colors suitable for thermal imaging, ranging from dark blue over purple to orange,
                   ///< yellow and white
        ,
        gpPolar  ///< Colors suitable to emphasize polarity around the center, with blue for negative, black
                 ///< in the middle and red for positive values
        ,
        gpSpectrum  ///< An approximation of the visible light spectrum (creates banding illusion but allows
                    ///< more precise magnitude estimates)
        ,
        gpJet  ///< Hue variation similar to a spectrum, often used in numerical visualization (creates
               ///< banding illusion but allows more precise magnitude estimates)
        ,
        gpHues  ///< Full hue cycle, with highest and lowest color red (suitable for periodic data, such as
                ///< angles and phases, see \ref setPeriodic)
    };
    ...
    void colorize(const double *data, const unsigned char *alpha, const QCPRange &range, QRgb *scanLine,
 ...
 public:
    ...
    */
    enum BracketStyle {
        ...
        ,
        bsEllipse  ///< An ellipse is drawn. The size of the ellipse is given by the bracket width/height
                   ///< properties.
        ,
...
class QCP_LIB_DECL QCPLegend : public QCPLayoutGrid {
    ...
    Q_PROPERTY(SelectableParts selectableParts READ selectableParts WRITE setSelectableParts NOTIFY
                   selectionChanged)
    Q_PROPERTY(
        SelectableParts selectedParts READ selectedParts WRITE setSelectedParts NOTIFY selectableChanged)
 ...
 public:
    ...
    */
    enum LineStyle {
        ...
        ,
        lsImpulse  ///< each data point is represented by a line parallel to the value axis, which reaches
                   ///< from the data point to the zero-value-line
    };
    Q_ENUMS(LineStyle)
...
class QCP_LIB_DECL QCPCurve : public QCPAbstractPlottable1D<QCPCurveData> {
 ...
 protected:
    ...
    QVector<QPointF> getOptimizedCornerPoints(int prevRegion, int currentRegion, double prevKey,
                                              double prevValue, double key, double value, double keyMin,
...
class QCP_LIB_DECL QCPStatisticalBox : public QCPAbstractPlottable1D<QCPStatisticalBoxData> {
 ...
 public:
    ...
    void addData(const QVector<double> &keys, const QVector<double> &minimum,
                 ...
                 const QVector<double> &upperQuartile, const QVector<double> &maximum,
                 bool alreadySorted = false);
    void addData(double key, double minimum, double lowerQuartile, double median, double upperQuartile,
...
class QCP_LIB_DECL QCPItemTracer : public QCPAbstractItem {
 ...
 public:
    /*!
      The different visual appearances a tracer item can have. Some styles size may be controlled with \ref
      setSize.
...
class QCP_LIB_DECL QCPPolarAxisRadial : public QCPLayerable {
 ...
 public:
    ...
    */
    enum AngleReference {
        arAbsolute  ///< The axis tilt is given in absolute degrees. The zero is to the right and positive
                    ///< angles are measured counter-clockwise.
        ,
        arAngularAxis  ///< The axis tilt is measured in the angular coordinate system given by the parent
                       ///< angular axis.
    };
 ...
 public:
    ...
    */
    ...
    double coordToAngleRad(double coord) const {
        return mAngleRad +
               (coord - mRange.lower) / mRange.size() * (mRangeReversed ? -2.0 * M_PI : 2.0 * M_PI);
    }  // mention in doc that return doesn't wrap
    double angleRadToCoord(double angleRad) const {
        return mRange.lower +
               (angleRad - mAngleRad) / (mRangeReversed ? -2.0 * M_PI : 2.0 * M_PI) * mRange.size();
    }
    void pixelToCoord(QPointF pixelPos, double &angleCoord, double &radiusCoord) const;
    QPointF coordToPixel(double angleCoord, double radiusCoord) const;
    SelectablePart getPartAt(const QPointF &pos) const;
...
class QCP_LIB_DECL QCPPolarGrid : public QCPLayerable {
 ...
 protected:
    ...
    void drawRadialGrid(QCPPainter *painter, const QPointF &center, const QVector<double> &coords,
                        const QPen &pen, const QPen &zeroPen = Qt::NoPen);
    void drawAngularGrid(QCPPainter *painter, const QPointF &center, double radius,
 ...
 public:
    ...
    */
    ...
    virtual double selectTest(const QPointF &pos, bool onlySelectable, QVariant *details = 0)
        const;  // actually introduced in QCPLayerable as non-pure, but we want to force reimplementation for
                // plottables
